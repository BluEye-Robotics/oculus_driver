...
class Message {
 public:
  // Only the SonarClient classes are able to modify this
 ...
 public:
  ...
  static Ptr Create(unsigned int size, const uint8_t* data,
    ...
    if (!header_valid(header) ||
        size != header.payloadSize + sizeof(OculusMessageHeader)) {
      return nullptr;
    }
    auto res = Create();
    res->header_ = header;
  ...
  bool is_ping_message() const {
    return this->message_id() == messageSimplePingResult;
  }
};
...
class PingWrapper {
 ...
 protected:
  ...
  explicit PingWrapper(const Message::ConstPtr& msg) : msg_(msg) {
    ...
    if (!msg_->is_ping_message()) {
      throw std::runtime_error(
          "Trying to make a PingMessage out of non-ping data.");
    }
  }
 ...
 public:
  ...
  uint32_t step() const {
    return ((this->has_gains() ? 4 : 0) +
            this->bearing_count() * this->sample_size());
  }
...
class PingWrapper1 : public PingWrapper {
 ...
 public:
  ...
  // virtual Ptr copy() const { return Create(this->msg_->copy()); } // not
  // compiling
  virtual PingWrapper::Ptr copy() const { return Create(this->msg_->copy()); }

  const OculusSimplePingResult& metadata() const {
    return *reinterpret_cast<const OculusSimplePingResult*>(
        this->msg_->data().data());
  }
...
class PingWrapper2 : public PingWrapper {
 ...
 public:
  ...
  const OculusSimplePingResult2& metadata() const {
    return *reinterpret_cast<const OculusSimplePingResult2*>(
        this->msg_->data().data());
  }
  ...
  // virtual bool    has_gains()   const { return
  // this->metadata().fireMessage.flags | 0x4; } // is broken
  virtual bool has_gains() const {
    return this->metadata().imageSize >
           this->sample_size() * this->bearing_count() * this->range_count();
  }
  virtual uint8_t master_mode() const {
    return this->metadata().fireMessage.masterMode;
  }
