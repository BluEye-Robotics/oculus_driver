...
//   unsigned       originalSize; // Size of the payload prior to any
//   compression unsigned       payloadSize;  // Size of the following payload
// };
...
public:
  ...
  struct TimeStamp {
    ...
    Message::TimePoint to_sonar_stamp() const {
      std::chrono::nanoseconds ns(1000000000 * this->seconds +
                                  this->nanoseconds);
    ...
    template <typename T> T to_seconds() const {
      return (T)seconds + 1.0e-9 * nanoseconds;
    }
  };
...
public:
  ...
  std::size_t
  read_next_item(uint8_t *dst) const; // data is assumed to have been reserved
