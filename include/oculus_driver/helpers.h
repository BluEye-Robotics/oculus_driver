...
inline void write_pgm(const std::string& filename, unsigned int width,
                      unsigned int height, const uint8_t* data) {
...
inline void write_pgm(const std::string& filename, unsigned int width,
                      unsigned int height, const float* data) {
  float m = data[0];
  float M = data[0];
...
inline void ping_data_to_array(T* dst, const OculusPingResultType& metadata,
...
inline std::vector<float> get_ping_acoustic_data(
    const std::vector<uint8_t>& pingData) {
  ...
  if (header.msgVersion != 2) {
    ...
    auto metadata =
        *reinterpret_cast<const OculusSimplePingResult2*>(pingData.data());
    std::vector<float> dst(metadata.nBeams * metadata.nRanges);
    ping_data_to_array(dst.data(), metadata, pingData);
    return dst;
  }
 ...
 */
template <typename T, class OculusPingResultType>
inline void get_ping_bearings(T* dst, const OculusPingResultType& metadata,
                              const std::vector<uint8_t>& pingData) {
  // copying bearing angles (
  auto bearingData =
      (const int16_t*)(pingData.data() + sizeof(OculusPingResultType));
  for (unsigned int i = 0; i < metadata.nBeams; i++) {
    dst[i] = (0.01 * M_PI / 180.0) * bearingData[i];
  }
}
inline std::vector<float> get_ping_bearings(
    const std::vector<uint8_t>& pingData) {
  auto header = *reinterpret_cast<const OculusMessageHeader*>(pingData.data());
  if (header.msgId != messageSimplePingResult) {
    throw std::runtime_error("Not a ping result");
...
inline std::pair<unsigned int, unsigned int> image_from_ping_data(
  ...
  for (unsigned int b = 0; b < metadata.nBeams; b++) {
    float bearingAngle =
        ((bearings.back() - bearings.front()) * b) / (metadata.nBeams - 1) +
        bearings.front();
    unsigned int idx = 0;
  ...
  for (unsigned int h = 0; h < height; h++) {
    ...
    for (unsigned int w = 0; w < width; w++) {
      ...
      if (range < 0.0f || range > get_range(metadata) ||
          abs(bearing) > 0.5f * aperture) {
        ...
        unsigned int bearingIndex =
            bearingLut[(metadata.nBeams - 1) * (bearing - bearings.front()) /
                       aperture];
        unsigned int rangeIndex =
            (metadata.nRanges - 1) * range / get_range(metadata);
        imageData[width * h + w] =
            pingData[metadata.nBeams * rangeIndex + bearingIndex];
      }
    }
  }
