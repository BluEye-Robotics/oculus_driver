...
AsyncService::AsyncService()
    : service_(std::make_unique<IoService>()), isRunning_(false) {}

AsyncService::~AsyncService() { this->stop(); }

AsyncService::IoServicePtr AsyncService::io_service() { return service_; }

bool AsyncService::is_running() const { return isRunning_; }

void AsyncService::start() {
  if (this->is_running()) return;
  std::cout << "starting" << std::endl;

  if (service_->stopped()) service_->reset();
