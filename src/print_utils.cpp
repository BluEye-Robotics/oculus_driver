...
std::string ip_to_string(uint32_t ip) {
  ...
  oss << (ip & 0x000000ff) << "." << ((ip & 0x0000ff00) >> 8) << "."
...
std::string mac_to_string(const uint8_t *mac) {
  std::ostringstream oss;
  oss << std::hex << (unsigned int)mac[0] << ":" << (unsigned int)mac[1] << ":"
...
std::string to_string(const OculusMessageHeader &msg,
  ...
  std::ostringstream oss;
  oss << prefix << "oculusId    : " << msg.oculusId << prefix
...
std::string to_string(const OculusStatusMsg &msg, const std::string &prefix) {
  std::ostringstream oss;
  oss << prefix << "deviceId        : " << msg.deviceId << prefix
      << "status          : " << msg.status << prefix
      << "part number     : " << msg.partNumber << prefix
      << "ipAddr          : " << ip_to_string(msg.ipAddr) << prefix
      << "ipMask          : " << ip_to_string(msg.ipMask) << prefix
      << "connectedIpAddr : " << ip_to_string(msg.connectedIpAddr) << prefix
      << "macAddr         : " << mac_to_string(&msg.macAddr0) << prefix
...
std::string to_string(const OculusSimplePingResult &msg,
                      const std::string &prefix) {
  std::ostringstream oss;
  oss << prefix << "pingId            : " << msg.pingId << prefix
      << "status            : " << msg.status << prefix
...
std::string to_string(const OculusSimpleFireMessage2 &msg,
  ...
  oss << prefix << "masterMode      : " << static_cast<int>(msg.masterMode)
      ...
      << "extFlags        : " << std::hex << msg.extFlags << prefix
      << "reserved        :";
...
std::string to_string(const OculusSimplePingResult2 &msg,
                      const std::string &prefix) {
  std::ostringstream oss;
  oss << prefix << "pingId            : " << msg.pingId << prefix
      << "status            : " << msg.status << prefix
...
std::ostream &operator<<(std::ostream &os, const OculusStatusMsg &msg) {
  ...
  os << "OculusStatusMsg :"
     << "\n- header :" << oculus::to_string(msg.hdr, prefix)
...
std::ostream &operator<<(std::ostream &os,
                         const OculusSimpleFireMessage2 &msg) {
  const std::string prefix("\n  - ");
